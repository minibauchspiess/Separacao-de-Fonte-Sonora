function [somFinal, somOrig, fs, pointBegin] = Filter2(audioFile)
...
A(winQtd+4)=0;
for i = 3:winQtd+2
    buffer = somOrig((winSize*(i-3)+1):winSize*(i-2));
    A(i) = max(abs(buffer))*2;
end
...
plot(somOrig);
hold on
plot(((cStart-3)*winSize+1):(cEnd-2)*winSize, somFinal);
hold off
